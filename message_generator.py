...
def update_source(filename: str):
    global messages
    make_uniq()
    with open(f'res/{filename}.txt', 'r') as file:
...
def write_source(raw_source: str):
    ...
    if len(res) / len(set(res)) < 2:
        ...
        return None
    # final adding
    with open('res/source.txt', 'a') as file:
        file.write(f'{new_source}\n')
        messages.append(new_source)


def word_count(tup: tuple or list, words: list) -> int:
    a, b = tup
...
def generate_text(max_len) -> str:
    ...
    def def_word_cut(_def_text: str) -> list:
        ...
        cuts = (None, None)
        while cuts.count(None) == 2 or (cuts.count(None) == 0 and abs(cuts[0] - cuts[1]) < 2) or 2 < word_count(cuts, words) < 15:
    ...
    while len(generated_message) < 10 or len(generated_message) > max_len:
